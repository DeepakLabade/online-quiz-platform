...
export default function QuizReviewPage() {
  ...
  useEffect(() => {
    async function fetchResults() {
      try {
        const res = await axios.get(`/api/student/quiz-result/${attemptId}`); //@ts-ignore
        setData(res.data);
      } catch (err) {
  ...
  if (loading)
    return (
      <div className="p-10 text-center animate-pulse text-gray-400 font-mono">
        Loading analysis...
      </div>
    );
      ...
      <div className="max-w-3xl mx-auto">
          ...
          <div className="text-right">
            ...
            <p
              className={`text-3xl font-black ${
                //@ts-ignore
                data.totalScore > 0 ? "text-blue-600" : "text-gray-900"
              }`}
            >
              {/*@ts-ignore */}
              {data.totalScore}
        ...
        <div className="space-y-6">
          ...
          {data.analysis.map((res, index) => (
            <div
              key={res.questionId}
              className="bg-white border border-gray-200 rounded-lg p-5 transition-hover hover:border-gray-300"
            >
              <div className="flex justify-between items-start gap-3">
                ...
                <span
                  className={`text-[10px] font-bold px-2 py-1 rounded whitespace-nowrap ${res.isCorrect ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                >
